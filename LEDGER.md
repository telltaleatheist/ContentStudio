...
**213. The inputs page offers "stories" or "chapters", and the choice follows what the item IS (Owen, 2026-09-25).** The "granular / broad chapters" selector goes; in its place, `stories` or `chapters`. "we'll generate stories for cases where i send a podcast episode through. it's a compilation of the week's videos and it goes to patreon and spreaker. and for everything else, we'll use chapters. they'll be single videos (sometimes an hour long for unfiltered, but they will always be 1 'story')." So a weekly podcast compilation is chaptered at the stories grain (#212's 45-second junction method) and every single video at the chapters grain (outline + assign), however long it is. The stored `chapterGrain` values `granular`/`broad` migrate to `chapters` with a logged line; the queue row and the job record carry the new value.
