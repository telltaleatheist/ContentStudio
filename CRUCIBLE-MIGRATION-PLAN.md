...
> **Rulings of 2026-09-25 (LEDGER #205), which override anything below that disagrees:** the inputs page UI is unfrozen; the Mac's Crucible is open to agents (the PC needs Owen's go every time); the editor Stories analyzer follows the chapters routing now; **`nomic-embed-text` is removed and P6 is deleted** (tag pools come from the chapter list); **Q14: no automatic hand-off**, the fast pin is the only route to the PC; **Q15: the official `qwen3-asr-1.7b`, never `-mlx`**. Crucible is at **1.0.34** on the Mac (checked 2026-09-25): job types `align, align-longform, asr, denoise, echo, load-model, load-voice, rvc, tts, unload-*`. Vendor the SDK at the version the adopt script names, repinned from Briefcase's 1.0.27 to the current tag.
