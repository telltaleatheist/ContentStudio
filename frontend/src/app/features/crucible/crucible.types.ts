/**
 * Crucible types: the renderer-side MIRROR of electron/crucible/wire.ts.
 *
 * Mirrored rather than imported because the renderer is a separate compilation unit (the
 * same pattern as publish.types.ts mirroring publish-types.ts). Unlike that pair, this one
 * cannot drift: tools/test-crucible-wire-mirror.js asserts that everything below this
 * header is byte for byte what electron/crucible/wire.ts says below its own, so the
 * cross-layer contract stays a TYPE (LEDGER Law 10). Change wire.ts, then copy its body
 * here; the keeper refuses any other edit.
 *
 * NOTHING HERE CARRIES A TOKEN: rows have tokenMasked, settings have keyHint, connect
 * codes come back elided.
 */
