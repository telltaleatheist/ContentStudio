...
export interface SourceSlot {
  ...
  isVideo: boolean;
  /**
   * The chosen file, or '' for "no file of its own". For a slot with a `masterQuadrant`
   * that means the master's quadrant stands in; for every other slot it means the source
   * is not part of the run at all.
   */
  path: string;
  /**
   * The master quadrant this video source is cropped from when it has no file — the
   * pipeline's long-standing fallback, named here so the empty choice can say so.
   * Null for audio slots and continuation parts, which have no such fallback.
   */
  masterQuadrant: string | null;
...
/**
 * This pipeline's own derived audio. Never selectable as an input: handing one back as a
 * source would make the run read and write the SAME file (AudioProcessor.get_processed_path
 * strips a trailing `_processed`, so the output path collapses onto the input) and would put
 * an already-ducked track through the automixer a second time.
 *
 * Reusing that work is still worth doing — which is what the "use the processed audio" option
 * is for. It keeps the ORIGINAL in the slot and tells the backend to adopt the sibling, so the
 * derived file is used without ever being treated as a source.
 */
const DERIVED_SUFFIX = /_processed\.[a-z0-9]+$/i;

/** The `_processed` sibling of a source file, by the same rule the backend writes it under. */
function processedSibling(path: string): string {
  const dot = path.lastIndexOf('.');
  const stem = dot > 0 ? path.slice(0, dot) : path;
  return `${stem.replace(/_processed$/i, '')}_processed.wav`;
}
...
export class ProjectSetupModalComponent implements OnInit, OnDestroy {
   ...
   * offered when EVERY assigned audio source has one (see `reusableCount`) — a run that
   * reused some sources and re-derived others would put ducked and un-ducked tracks in the
  ...
  private async listCandidates(master: string): Promise<FileCandidate[]> {
    ...
    // Derived audio is recorded rather than merely discarded: it is not selectable, but its
    // presence is what makes reusing the previous run's work possible.
    this.derivedFiles = new Set(paths.filter(p => DERIVED_SUFFIX.test(p)));

    return paths
      .filter(p => !DERIVED_SUFFIX.test(p))
  ...
  /** How many slots are feeding the run from a file of their own. */
  get assignedCount(): number {
    return this.slots.filter(s => s.path).length;
  }

  /**
   * Video slots running on the master's quadrant instead of a file. Counted separately from
   * `assignedCount` because they cost nothing to detect and change nothing about the run's
   * inputs — but they are NOT missing picture, and the header should not imply they are.
   */
  get quadrantCount(): number {
    return this.slots.filter(s => !s.path && s.masterQuadrant).length;
  }

  /**
   * What the empty choice means for THIS slot. For the four sources the master carries in
   * its own frame, "no file" is a real, working answer — the picture comes from the master
   * cropped to that quadrant — so the dropdown names it rather than reading "None".
   */
  emptyOptionLabel(slot: SourceSlot): string {
    return slot.masterQuadrant ? `Master — ${slot.masterQuadrant} quadrant` : '— None —';
  }

  /** The tooltip for a slot's file choice, empty or not. */
  slotTitle(slot: SourceSlot): string {
    if (slot.path) return slot.path;
    if (slot.masterQuadrant) {
      return `No separate file — this picture is taken from the ${slot.masterQuadrant} ` +
             `quadrant of the master, at the master's own timing (no offset, no drift correction).`;
    }
    return 'Not used in this run';
  }

  /** True when the slot has no file AND nothing stands in for it: genuinely out of the run. */
  isDropped(slot: SourceSlot): boolean {
    return !slot.path && !slot.masterQuadrant;
  }

  // ── Re-processing ───────────────────────────────────────────────────────────

  /**
   * This project has already been through the pipeline, so pressing Process REPLACES a
   * session rather than creating one. Derived from the scan, not passed in: the state on disk
   * is the only thing that decides it, and it cannot then disagree with the row's badge.
   */
  get isReprocess(): boolean {
    const state = this.entry?.scan?.state;
    return state === 'processed' || state === 'edited';
  }

  /** True when this project carries edits/transcript that the re-run will discard. */
  get hasEdits(): boolean {
    return this.entry?.scan?.state === 'edited';
  }

  get hasTranscript(): boolean {
    return !!this.entry?.scan?.hasTranscript;
  }

  /** The audio slots feeding this run — the ones reuse has to account for. */
  private get assignedAudioSlots(): SourceSlot[] {
    return this.slots.filter(s => !s.isVideo && s.path);
  }

  /** How many of them already have their derived audio sitting next to them. */
  get reusableCount(): number {
    return this.assignedAudioSlots.filter(s => this.derivedFiles.has(processedSibling(s.path))).length;
  }

  /**
   * Whether to offer reuse at all. ALL-OR-NOTHING by design: the automixer balances tracks
   * against each other in one pass, and the files on disk already carry that balance, so a
   * mix of reused and freshly-derived tracks would be ducked against nothing.
   */
  get canReuseProcessed(): boolean {
    const audio = this.assignedAudioSlots;
    return this.isReprocess && audio.length > 0 && this.reusableCount === audio.length;
  }

  /**
   * Reuse as it will ACTUALLY be sent. The checkbox alone is not enough: changing a source
   * after ticking it can take the option away (a slot whose derived file is missing), and the
   * stale `true` would otherwise keep the automixer disabled with an explanation for an option
   * no longer on screen. Everything that depends on reuse reads this, including onProcess.
   */
  get reuseActive(): boolean {
    return this.reuseProcessed && this.canReuseProcessed;
  }

  /** Toggling reuse off restores the automixer default; on, it is not available at all. */
  onReuseProcessedChange(on: boolean): void {
    this.reuseProcessed = on;
    this.error = null;
  }

  /**
   * What pressing Process will throw away. Only the sidecars written in TIMELINE coordinates
   * are listed, because those are the ones a rebuilt timeline invalidates.
   */
  get clearsOnProcess(): string[] {
    const out: string[] = [];
    if (this.hasEdits) out.push('the cuts, blades and stories saved for this session');
    if (this.hasTranscript) out.push('its transcript');
    return out;
  }
  ...
  async onProcess(): Promise<void> {
    ...
    try {
      ...
      const { options, errors } = buildWorkflowOptions({
        ...
        // The EFFECTIVE value, re-evaluated here, so a slot changed after ticking the box
        // cannot smuggle a reuse through for a source that no longer qualifies.
        reuseProcessedAudio: this.reuseActive
      });
      if (!options) {
      ...
      //
      // Cleared only once the run is genuinely UNDERWAY. Doing it first would mean a start
      // that then failed had destroyed a session's edits for a run that never happened — and
      // the run needs twelve minutes before it replaces the zip, so there is no race in
      // waiting. A clear that fails cannot un-start the job: it is reported and the run
      // continues, because the alternative is a silent stale sidecar.
      const zipPath = this.entry?.scan?.zipPath;
      if (this.isReprocess && zipPath) {
        try {
          const { removed } = await this.host.clearEditorSessionState({ zipPath });
          this.clearedNote = removed.length ? `Cleared ${removed.join(' and ')}.` : null;
        } catch (err: any) {
          this.error = `Processing started, but the previous session's edits could not be ` +
                       `cleared: ${err?.message || String(err)}. Delete them by hand before ` +
                       `opening the rebuilt session — they refer to the old timeline.`;
        }
      }
    } finally {
      this.starting = false;
