...
/**
 * Where each video source lives inside the MASTER's four-up recording, and therefore what
 * the pipeline uses when that source has no file of its own: every compound generator
 * already falls back to the master cropped to this quadrant (`if cam1_asset_id: … else:
 * crop the master`), with no offset and no retime applied, because the master IS the
 * reference — a crop of it cannot drift from it.
 *
 * So "no file" for one of these four never means "no picture". It means "picture from the
 * master", which is exactly what you want when a capture was lost, split, or is simply not
 * worth using. The UI reads this map so a slot says which quadrant will stand in for it
 * instead of the flatly wrong "None".
 *
 * Continuation parts (screenVideo2…) are deliberately absent: they extend a capture that
 * exists, and there is no second screen quadrant to fall back to.
 */
export const MASTER_QUADRANTS: { [key: string]: string } = {
  screenVideo: 'top-left',
  cam1: 'bottom-left',
  cam2: 'top-right',
  gameVideo: 'bottom-right'
};
