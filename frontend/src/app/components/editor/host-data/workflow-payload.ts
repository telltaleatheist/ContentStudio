...
import { AudioSource, VIDEO_CONTINUATION_PARTS } from './types';
...
export interface WorkflowPayloadSpec {
  ...
  /**
   * Re-process only: adopt the `_processed` audio already on disk instead of extracting,
   * aligning and ducking every source again. See the payload field of the same name.
   */
  reuseProcessedAudio?: boolean;
}
...
export interface WorkflowOptionsPayload {
  ...
  alignmentOverrides: AlignmentOverrides | null;
  /**
   * Adopt each source's existing `<stem>_processed.wav` verbatim rather than deriving it
   * again. Absent (the normal case) means a full derivation, exactly as before.
   *
   * The rows still carry the ORIGINAL files: the backend derives the processed sibling with
   * the same helper that wrote it, so the two cannot disagree about the name, and it verifies
   * against the alignment sidecar that the run which produced it actually finished.
   */
  reuseProcessedAudio?: boolean;
}
...
export function buildWorkflowOptions(spec: WorkflowPayloadSpec): WorkflowPayloadResult {
  ...
  const options: WorkflowOptionsPayload = {
    ...
    // Reused audio has ALREADY been through the automixer — dugan-automixer.ts renames its
    // output over the `_processed` file in place, so what is on disk carries the gain. Running
    // it again would duck already-ducked tracks. Forced here as well as in the UI: this is the
    // one place both entry points funnel through, so it is the one place the rule cannot be
    // bypassed by a caller that forgot it.
    autoDuck: spec.reuseProcessedAudio ? false : spec.autoDuck,
