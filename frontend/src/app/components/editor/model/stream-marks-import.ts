...
//
// THE OTHER IDEA WORTH KNOWING: the timeline is NOT the recording. See masterToTimeline —
// three clocks meet here (the stream's elapsed seconds, the master file's seconds, the
// timeline's seconds) and only the first two are separated by a constant.
//
 ...
 *
 * A mark is a moment in the RECORDING. The editor's timeline is not the recording: the
 * processing step drops dead air, so the timeline is a NON-LINEAR remap of the master, and
 * the further into the night you go the further the two clocks have drifted apart.
 *
 ...
 *
 * Six marks, mapped through the segment table against what a single additive offset produced
 * for the same night (he had tuned the offset until the LAST story landed):
 *
 ...
 * right; the arithmetic was not. So the marks travel through the segment table, and the single
 * constant that remains (`streamStartInMaster`) is the one thing that genuinely IS constant —
 * the moment the stream's clock started, in the recording's own seconds.
 *
 * THE SEGMENTS MUST BE ORDERED BY sourceStart before any of this is called: these run per
 * mouse move over 1954 segments × 20 marks, so they binary-search and do not sort. Use
 ...
 * `t` that is ON the timeline, which is what the drag needs: the pointer is a timeline
 * position and the answer it has to produce is a position in the recording.
 *
