...
export class WhisperBridge extends EventEmitter {
  ...
  constructor(config: WhisperConfig) {
    super();
    this.config = config;
    log.info(`[WhisperBridge] Initialized with binary: ${config.binaryPath}`);
    log.info(`[WhisperBridge] Models directory: ${config.modelsDir}`);
  }
  ...
  getModelPath(modelName: string = WhisperBridge.DEFAULT_MODEL): string {
    ...
    if (!WhisperBridge.AVAILABLE_MODELS.includes(normalizedName)) {
      log.warn(`[WhisperBridge] Invalid model: ${modelName}, using ${WhisperBridge.DEFAULT_MODEL}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on('close', (code) => {
        ...
        if (processInfo.aborted) {
          log.info(`[WhisperBridge] [${processId}] Aborted after ${duration}ms`);
          resolve({
            processId,
            success: false,
            exitCode: code,
        ...
        if (code === 0) {
          ...
          if (!fs.existsSync(srtPath)) {
            log.error(`[WhisperBridge] [${processId}] No SRT file created at ${srtPath}`);
            resolve({
              processId,
              success: false,
              exitCode: code,
          ...
          log.info(`[WhisperBridge] [${processId}] Completed successfully in ${duration}ms`);
          resolve({
            processId,
            success: true,
            exitCode: code,
          ...
          log.error(`[WhisperBridge] [${processId}] Failed with code ${code}`);
          log.error(`[WhisperBridge] [${processId}] stderr: ${stderrBuffer.slice(-500)}`);
          resolve({
            processId,
            success: false,
            exitCode: code,
      ...
      proc.on('error', (err) => {
        ...
        log.error(`[WhisperBridge] [${processId}] Spawn error: ${err.message}`);
  ...
  abort(processId: string): boolean {
    ...
    if (!processInfo) {
      log.warn(`[WhisperBridge] Cannot abort ${processId}: not found`);
      return false;
    }

    log.info(`[WhisperBridge] [${processId}] Aborting transcription`);
   ...
   */
  abortAll(): void {
    log.info(`[WhisperBridge] Aborting all ${this.activeProcesses.size} transcriptions`);
