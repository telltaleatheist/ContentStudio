...
/**
 * What the story analysis handlers need from the host that is not the store: where the prompt
 * assets live. AIManagerService's constructor loads them whatever the caller intends to ask
 * for, and ipc-handlers.ts is the one place that knows the directory — passed in rather than
 * recomputed here, so the two cannot drift (Law 10).
 */
 ...
 * temperature 0, `format: "json"`, num_predict — do not travel through this door:
 ...
 * plain answer either way. The pipeline and its prompts are otherwise untouched; only the
 * transport changed.
 */
function setupStoryAnalysisHandlers(store: Store<any>, deps: EditorIpcDeps): void {
   ...
   * to the renderer rather than a transport message.
   */
