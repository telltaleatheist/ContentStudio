 ...
 *
 * Ported from Briefcase's backend/src/crucible/transport-failure.ts
 * (BookForge's electron/crucible/transport-failure.ts before it). The SDK
 * maps a connection it never got onto `CrucibleUnreachable`. What it does NOT
 * map is a socket destroyed AFTER the response started: undici surfaces that
 * as a bare `TypeError: terminated` (and a connect that failed inside `fetch`
 * as `TypeError: fetch failed` with the errno on `cause.code`). Those must be
...
export function transportFailureCause(err: unknown): string | null {
  ...
  // realm, which is not always the realm a test harness calls Error.
  if (typeof err !== 'object' || err === null) return null;
