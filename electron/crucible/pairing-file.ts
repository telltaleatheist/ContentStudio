 ...
 *
 * Ported from Briefcase's backend/src/crucible/pairing-file.ts (BookForge's
 * electron/crucible/pairing-file.ts before it). It is this module's own
 * SYNCHRONOUS execution of the SDK's one path rule, not a second opinion: the
 * three names the path is composed from are the SDK's constants. The SDK's
 * `readPairingFile` is async for a bundling reason that does not apply here,
 * and the pane's discovery row composes this beside other synchronous reads.
 *
 ...
 * install a second one over the top of one that is already running (Law 1).
 */
