 ...
 *
 * Ported from Briefcase's backend/src/crucible/discovery.ts (BookForge's
 * electron/crucible/discovery.ts before it), keeping its one question: what
 * would the registry row for the Crucible on this computer look like? A local
 * server is not a different KIND of server; it becomes a row only through the
 * same add door every other server uses, and refusing the offer is an
 * ordinary state.
 *
 ...
 * ("deleted when the host lands"). Every Crucible ContentStudio can meet
 * (1.0.x) writes the pairing file, including the Windows host, so this reads
 * only that, and needs no TOML parser and no WSL distro setting.
 */
...
import type { DiscoveredCrucibleRow } from './wire';
