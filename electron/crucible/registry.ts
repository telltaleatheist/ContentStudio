 ...
 *
 * Ported from Briefcase's backend/src/crucible/registry.ts (itself BookForge's
 * electron/crucible/servers.ts). One kind of server: a name, a URL and a
 * token, whether it answers on 127.0.0.1 or across the tailnet. Nothing here
 * asks which (BookForge's ruling of 2026-09-15: a local server is not a
 ...
 *
 * No fallbacks (Law 1): an unknown name is refused, not answered with a
 * default; a duplicate name or address is refused, not replaced; a URL without
 * a scheme is refused, not prefixed with a guessed `http://`; a corrupt file
 * is refused, never overwritten.
 */
import * as fs from 'fs';
import * as path from 'path';
import { CrucibleRegistryError } from './errors';
import type { CrucibleServerRow } from './wire';
 ...
 * one string, and P3's lane ids hang off it as `gpu:<name>`, which is why a
 * colon is refused.
 */
...
/** The registry over one file. The context binds it to userData; a keeper binds it to a temp dir. */
