 ...
 * means for a person.
 *
 * Ported from Briefcase's install/engine-presence.ts (BookForge's
 * electron/crucible/engine-presence.ts before it). A local Crucible is an OS
 * service no app owns (INTEGRATING-AN-APP.md §4.4, plan section 5): an app
 ...
 * `status --json` / `start --json`, through the runner, so a keeper drives
 * them with a scripted runner and a temp CRUCIBLE_HOME.
 */
import type { LocalStatus, Runner } from '@crucible/bootstrap';
import type { CrucibleEnginePresence } from './wire';
...
/** The real controls, over the bootstrap's own process runner. */
...
export function presenceOf(observed: LocalStatus): CrucibleEnginePresence {
  ...
  switch (observed.state) {
    ...
    case 'unauthorized':
      return {
        ...base,
        message: 'ContentStudio is not authorised to use the Crucible on this computer. Connect it again in Settings › Crucible Servers.',
        offerStart: false,
      };
    case 'wrong_service':
