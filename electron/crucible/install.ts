 ...
 *
 * Ported from Briefcase's install/install.ts (BookForge's
 * electron/crucible/install.ts before it: releaseToInstall,
 * runningCrucibleVersion, hostabilityOf, crucibleHostFacts,
 * driveCrucibleInstall). Everything that reads the machine or the network is
 * injected, so a keeper drives every branch with no GPU, no installer and no
 * GitHub.
 *
 * The rules, from crucible docs/INTEGRATING-AN-APP.md §4 and plan section 5:
 *
 *  - ONE CRUCIBLE PER MACHINE, shared with BookForge, Foundry and Briefcase.
 *    The never-older gate runs before anything is spawned: never install over
 *    a newer running Crucible, never reinstall the same one. No `--force`.
 ...
 *    another computer".
 *  - Install a BARE service (`jobTypes: ['echo']`). What ContentStudio needs
 *    arrives later through the module (plan section 3.4), after the user's
 *    routing choices, so nobody downloads a 27B they were about to route to
 *    Claude.
 *  - On Owen's Mac the face is ADOPT: it already runs Crucible as a launchd
 *    service. ContentStudio never installs over it, never stops it, never
 *    uninstalls it. The PC is paired to, never installed on.
 */
...
export type InstallPlatform = CrucibleInstallPlan['platform'];
 ...
 * interface so a keeper can put a channel at 1.0.22 in front of an engine at
  ...
  if (sources.compare(latest, running) < 0) {
    throw new CrucibleInstallError(
      ...
      `the release channel's latest is ${latest} and Crucible ${running} is running on this computer; `
        ...
        + 'BookForge, Foundry and Briefcase, and ContentStudio never takes it backwards.',
    );
  }
  if (sources.compare(latest, running) === 0) {
    throw new CrucibleInstallError(
...
/** The real machine. `discovered` is the pairing-file reader, passed in. */
...
export function crucibleHostFacts(host: InstallHost): CrucibleHostFacts {
  ...
  const refuse = (code: string, message: string, options: { command?: string; detail?: string } = {}): void => {
...
/**
 * COULD A CRUCIBLE LIVE HERE, and why. BookForge's rule, adopted by Briefcase
 * and the plan (section 5): Apple silicon and NVIDIA hosts install; an Intel
 * Mac and Linux without NVIDIA connect to another computer instead; Windows
 * always installs (its native engine runs on anything, and the faster Linux
 * engine follows by itself where WSL2 can host it).
 */
  ...
  if (facts.platform === 'other') {
    return {
      ...
      why: `Crucible runs on Apple silicon Macs, Windows, and Linux with an NVIDIA card, and ${facts.platformName} is none of them. `
        + 'Connect ContentStudio to a Crucible on another computer instead.',
    };
  }
...
function stepsFor(facts: CrucibleHostFacts): CrucibleInstallStep[] {
  ...
  const steps: CrucibleInstallStep[] = [
    ...
    { title: 'Starting Crucible', detail: 'ContentStudio waits until the engine answers.', done: false },
  ];
  if (facts.platform === 'win32') {
    steps.push({
  ...
  steps.push(
    { title: 'Connecting ContentStudio', detail: 'ContentStudio adds it to Settings › Crucible Servers.', done: false },
    {
      title: 'Preparing what ContentStudio needs',
      detail: 'Transcription, chaptering and the metadata models, installed once you have chosen your routing. Several '
        + 'gigabytes on a first setup; it keeps going while you work.',
      done: false,
    },
  );
  return steps;
...
/** The part of `@crucible/bootstrap` this app drives. Injected so a keeper never installs anything. */
...
/**
 * The options ContentStudio hands the package: a BARE service (`['echo']`),
 * and where to send the output. No release: the gate fills it from the channel.
 */
export function contentStudioInstallOptions(
