 ...
 * ContentStudio follows an orchestrator's hop.
 *
 * Ported from Briefcase's backend/src/crucible/engine-resolve.ts (BookForge's
 * electron/crucible/engine-resolve.ts before it). crucible
 ...
 * followed, so a misconfigured chain cannot loop (plan section 4: owens-pc's
 * engine may sit behind the tray, and the hop is followed once, cached 60 s).
 *
    ...
    if (behind.role !== 'engine') {
      throw new EngineResolveError(
        ...
        `"${entry.name}" (${entry.url}) is an orchestrator whose engine at ${ref.url} answers "${behind.role}", not "engine". `
          + 'ContentStudio follows one hop and no more.',
      );
    }
