// READ-ONLY REFERENCE (LEDGER #212, P8c). Restored verbatim from 4ba2580:electron/services/editor/chapter-splitter.ts
// (deleted in P8b, d409cdf). It is the METHOD SPEC for the stories grain (45 s stretches, a rating per
// junction ranked never thresholded, duration-derived over-segmentation with a minimum gap, placement at
// the turn's sentence, adjacent-pair consolidation), re-done on snap in
// electron/services/metadata/chaptering/stories.ts. It is outside the build (tsconfig includes electron/
// only), imports nothing that still exists, and must never be called or copied back into electron/.
// It stays until the snap version matches it on the 2026-09-23 stream (#212).
