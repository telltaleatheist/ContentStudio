...
class ManifestBuilder:
    ...
    def build_tracks(self, total_declared, frame_seconds):
        ...
        # Master segments across layers OVERLAP as a matter of course, and that is not an
        # error: every video source without a capture file of its own is drawn as the master
        # cropped to that source's quadrant (screen top-left, cam 1 bottom-left, cam 2
        # top-right, game bottom-right). A session that fell back for two of them — say the
        # screen capture was unusable and there is no game capture — puts two or three
        # master references on screen at the same instant, in different compounds and
        # different lanes. Measured on the 2026-08-12 session: 1677 cuts, each carrying THREE
        # master references (GS screen, GS game, SSB screen), all with byte-identical
        # timeline and source times.
        #
        # They are the same recording playing at the same moment, so the editor's video
        # track wants ONE segment. What must not differ is WHICH PART of the master each
        # claims is playing: two references covering the same instant have to map it to the
        # same point in the file, because the track can only scrub to one of them. So the
        # check moves from "do they overlap" (routine) to "do they agree" (never routine) —
        # agreeing references collapse into one segment spanning both, and a genuine
        # disagreement still fails loud, naming both sides and their source times.
        merged = []
        for seg in master_segs:
            prev = merged[-1] if merged else None
            if prev is not None and seg['timeline_start'] < prev['timeline_end']:
                # timeline_start - source_start is the segment's constant shift between
                # timeline time and master-file time. Equal shifts = the same playback.
                if (seg['timeline_start'] - seg['source_start']
                        != prev['timeline_start'] - prev['source_start']):
                    raise ManifestError(
                        f"master video segments disagree at {float(seg['timeline_start'])}s — "
                        f"{prev['label']} (layer {prev['layer']}) plays "
                        f"{Path(master_file).name} from {float(prev['source_start'])}s while "
                        f"{seg['label']} (layer {seg['layer']}) plays it from "
                        f"{float(seg['source_start'])}s at the same instant; the master "
                        "cannot be in two places at once")
                if seg['timeline_end'] > prev['timeline_end']:
                    prev['timeline_end'] = seg['timeline_end']
                continue
            merged.append(dict(seg))   # copy: self.leaves must stay as flattened
        if len(merged) != len(master_segs):
            print(f"[editor_manifest] {len(master_segs)} master video references collapsed to "
                  f"{len(merged)} segments (quadrant fallbacks of the same recording)",
                  file=sys.stderr)
        master_segs = merged
