...
def _completed_alignment_audio(master_video):
    ...
    `<clean_name>_alignment.json` is written at the very end of a successful run, after every
    source has been aligned, so a type listed in it is proof that the `_processed` file beside
    that source is the finished article and not the debris of a run that died halfway.

    The whole entry is kept, not just the name: a reused source measured nothing this run, and
    the offset it was aligned by has to be carried into the new sidecar or it would vanish from
    the record — taking with it both the transcript editor's word-placement data and the ability
    to reuse the same file a second time.

    Returns {} when there is no sidecar (never run, or the run failed). A sidecar that exists
    but cannot be read is a REAL error and propagates — treating unreadable provenance as "no
    provenance" would let a reuse fall through to the re-derivation the user asked to skip.
    """
    session_name = Path(master_video).stem.replace(' master', '')
    sidecar = Path(master_video).parent / f"{session_name.replace(' ', '_')}_alignment.json"
    ...
    try:
        ...
        raise RuntimeError(
            f"alignment sidecar {sidecar.name} could not be read ({e}) — it is the record of "
            f"which processed audio is complete, so reuse cannot be verified against it") from e
    return {s['type']: s for s in payload.get('sources', [])
            if isinstance(s, dict) and s.get('kind') == 'audio' and s.get('type')}


def _reuse_processed_audio(audio_type, normalized_type, audio_path, audio_processor,
                           completed_types):
    """Adopt the `_processed` file that already sits beside `audio_path`.

    Three things must hold, and each failure names itself rather than quietly re-deriving —
    the user asked to skip an hour of work, so silently doing it anyway is as wrong as
    silently using a stale file:

      1. The last completed run aligned this source (it appears in the alignment sidecar).
      2. Its `_processed` sibling exists. The path comes from the SAME helper that writes it
         (AudioProcessor.get_processed_path), so the reader and the writer cannot disagree
         about the name.
      3. That file is NEWER than the source. An older one predates the recording it claims to
         be derived from, which means the source was replaced since.

    Returns the processed_audio entry. `is_reused` marks it for the sidecar, which must not
    re-report an offset this run never measured.
    """
    if normalized_type not in completed_types:
        raise RuntimeError(
            f"cannot reuse processed audio for {audio_type}: the alignment sidecar does not "
            f"list '{normalized_type}', so no completed run produced it. Process it normally.")

    processed = Path(audio_processor.get_processed_path(audio_path, '.wav'))
    if not processed.exists():
        raise RuntimeError(
            f"cannot reuse processed audio for {audio_type}: {processed.name} is not on disk "
            f"next to {Path(audio_path).name}. Process it normally.")

    source_mtime = Path(audio_path).stat().st_mtime
    if processed.stat().st_mtime < source_mtime:
        raise RuntimeError(
            f"cannot reuse processed audio for {audio_type}: {processed.name} is OLDER than "
            f"{Path(audio_path).name}, so it was derived from a different recording.")

    previous = completed_types[normalized_type]
    duration, sample_rate, channels = audio_processor.get_audio_info(str(processed))

    # The offset this file was ALREADY aligned by, carried forward verbatim. Nothing is
    # measured here; the number is the previous run's, and the sidecar will say so.
    sync_info = {'is_reused': True, 'offset_seconds': previous['offsetSeconds']}
    for key, dest in (('confidence', 'correlation_score'), ('trusted', 'alignment_trusted')):
        if key in previous:
            sync_info[dest] = previous[key]

    # duration is an FCPXML RATIONAL string, not seconds ("393157830/30000s") — that is what
    # AudioProcessor.get_audio_info returns and what the generators consume. Passed on
    # untouched, so this entry is shaped exactly like a freshly derived one, and printed
    # verbatim rather than formatted as a number it is not.
    print(f"\n{'='*60}", file=sys.stderr)
    print(f"Reusing {audio_type}: {processed.name}", file=sys.stderr)
    print(f"  duration {duration}, aligned by a previous run at "
          f"{previous['offsetSeconds']:.3f}s — not re-derived", file=sys.stderr)
    print(f"{'='*60}\n", file=sys.stderr)
    return {
        'path': str(processed),
        'duration': duration,
...
def _write_alignment_sidecar(output_dir, clean_name, master_video, processed_audio,
    ...
    for atype, info in processed_audio.items():
        ...
        if sync_info.get('is_reused'):
            # Carried from the run that actually measured it. Named distinctly so the record
            # never implies this run re-measured anything.
            method = 'reused-from-previous-run'
            drift = None
...
def denoise_mic_audio(audio_type, audio_path, audio_processor):
    """Isolate the speaker's voice on a mic track, returning the cleaned track's path.

    The model runs on a Crucible (LEDGER #200): core/voice_separation.py plans the
    chunks, skips the silent ones and reassembles the stems HERE, and asks
    Electron's main process for each chunk with a `separation_request` on this
    process's stdout; main runs the `denoise` job and answers on stdin.

    FAIL LOUD: a failed job, a server that cannot isolate voice, or any other
    error aborts the whole run. We never fall back to the noisy original, because
    that would silently ship un-isolated audio the user explicitly asked to clean."""
    from core.voice_separation import isolate_voice, VoiceIsolationError
    ...
    # Voice isolation is slow (~1 min per 6-min section), so it gets a real,
    # advancing sub-progress bar. voice_separation.py reports its PLAN / CHUNK
    # lines (and mirrors them to stderr for the detailed log); they are turned
    # into emit_progress(sub_progress=...) here. Main moves the bar WITHIN a
    # chunk from the job's own events (python-service.ts), between these lines.
    import re
    plan = {'sections': None, 'minutes': None}

    def report(line):
        if line.startswith('PLAN'):
            mtot = re.search(r'->\s*(\d+)\s*chunks', line)
            mmin = re.search(r'([\d.]+)\s*min input', line)
            plan['sections'] = int(mtot.group(1)) if mtot else None
            plan['minutes'] = float(mmin.group(1)) if mmin else None
            if plan['sections']:
                emit_progress(30, f"Isolating voice on {mic_label} — 0 of {plan['sections']} "
                              f"sections ({plan['minutes']:.0f} min of audio)", sub_progress=0)
            return
    ...
    try:
        isolate_voice(str(src), str(cleaned), ffmpeg=ffmpeg, ffprobe=ffprobe, track=mic_label,
                      parent_pid=_PARENT_PID, report=report)
    except VoiceIsolationError as err:
        emit_error(f"Voice isolation failed for {audio_type}: {err}")
        raise RuntimeError(f"Voice isolation failed for {audio_type}: {err}") from err
    if not cleaned.exists():
        emit_error(f"Voice isolation for {audio_type} finished without writing {cleaned.name}")
        raise RuntimeError(f"Voice isolation for {audio_type} wrote no output")
...
def main():
    ...
    try:
        ...
        # Voice isolation: remove background from mic1/mic2 BEFORE alignment.
        # denoise_mics comes from the workflow checkbox, which is only offered when
        # the selected Crucible can isolate voice (LEDGER #200); the model runs
        # there, one `denoise` job per chunk, answered by Electron on stdin.
        denoise_mics = data.get('denoiseMics', False)

        # RE-PROCESS REUSE: adopt each source's existing `_processed` audio instead of
        # extracting, aligning and ducking it again. A completed run leaves those files
        # beside the sources; re-deriving them from unchanged inputs reproduces the same
        # numbers at the cost of an hour, so a re-process may skip straight to them.
        #
        # Refused together with the automixer. dugan-automixer.ts renames its output OVER
        # the `_processed` file, so a file on disk from a ducked run already carries the
        # gain; ducking it again would compress what is already compressed. The payload
        # builder forces autoDuck false alongside this flag — a payload carrying both means
        # something bypassed it, and guessing which the user meant is not this script's job.
        reuse_processed_audio = bool(data.get('reuseProcessedAudio', False))
        if reuse_processed_audio and auto_duck:
            msg = ("reuseProcessedAudio and autoDuck were both requested. The processed audio "
                   "on disk already has the automixer applied (Dugan writes over the "
                   "_processed file), so running it again would duck already-ducked tracks.")
            emit_error(msg)
        ...
        # What the last COMPLETED run aligned, read once. Empty unless reuse was asked for —
        # there is no reason to open the sidecar on an ordinary run, and an unreadable one
        # must not fail a run that never intended to consult it.
        completed_alignment_audio = (
            _completed_alignment_audio(master_video) if reuse_processed_audio else {})
        if reuse_processed_audio and not completed_alignment_audio:
            msg = ("reuseProcessedAudio was requested but no alignment sidecar was found next "
                   "to the master. Nothing here proves a previous run finished, so there is no "
                   "processed audio that can be trusted. Process normally instead.")
            emit_error(msg)
        ...
        # Reuse bypasses the unified soundboard path as well: it re-measures every SB track
        # together and rewrites their `_processed` files, which is the exact work being
        # skipped. Each SB source is adopted individually in the loop below instead.
        if soundboard_files and use_advanced_sync and not reuse_processed_audio:
        ...
        for audio_type, audio_path in audio_sources_input.items():
            if audio_path:
                ...
                # REUSE (before everything else): the finished `_processed` file already
                # embodies isolation, alignment and ducking, so every step below would be
                # re-derivation of work being deliberately skipped. Placed ahead of the
                # output-file short-circuit because a raw source type never reaches that
                # branch, and reuse has to apply to exactly those raw types.
                if reuse_processed_audio:
                    processed_audio[normalized_type] = _reuse_processed_audio(
                        audio_type, normalized_type, audio_path, audio_processor,
                        completed_alignment_audio)
                    continue
                ...
                if (denoise_mics
                    ...
                    audio_path = denoise_mic_audio(
                        audio_type, audio_path, audio_processor
                    )
